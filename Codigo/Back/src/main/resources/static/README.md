...
A maioria é executada uma única vez, manualmente, no banco de cada ambiente (hml/prod) — exceto as duas marcadas "auto" abaixo, que são idempotentes (`IF NOT EXISTS`/`WHERE ... IS NULL`, viram no-op depois da 1ª vez) e rodam sozinhas em todo start via `spring.sql.init.schema-locations` (ver `application.properties`), antes do Hibernate `ddl-auto=update`. As demais não foram convertidas para esse mecanismo porque não são todas seguras para reexecução — ver item E-C1 no `AUDITORIA.md` sobre adotar uma ferramenta de migração de verdade (Flyway) para o conjunto todo.
