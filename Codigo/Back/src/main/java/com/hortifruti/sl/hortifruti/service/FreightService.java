...
public class FreightService {
  ...
  public double calculateFreight(FreightCalculationRequest request) {
    ...
    double fuelCostPerKm = freightProperties.getVehicle().getFuelPrice() /
        freightProperties.getVehicle().getKmPerLiterConsumption();

    double operationalCostPerKm = fuelCostPerKm +
    ...
    double monthlyDeliveryPersonCost = freightProperties.getDeliveryPerson().getBaseSalary() *
        (1 + freightProperties.getDeliveryPerson().getChargesPercentage() / 100);

    double hourlyDeliveryPersonCost = monthlyDeliveryPersonCost /
        freightProperties.getDeliveryPerson().getMonthlyHoursWorked();

    double finalHourlyCost = hourlyDeliveryPersonCost *
    ...
    int estimatedTimeMinutes = Integer.parseInt(request.estimatedTimeMinutes().replaceAll("[^0-9]", ""));
    double totalVariableCost = (operationalCostPerKm * distanceKm) +
        (costPerMinute * estimatedTimeMinutes);

    // 4. Apply margin and fixed fee
    double margin = totalVariableCost *
        (freightProperties.getMargin().getMarginPercentage() / 100);

    double finalFreight = totalVariableCost + margin +
        freightProperties.getMargin().getFixedFee();
