...
public class BulkNotificationService {
    ...
    public BulkNotificationResponse sendBulkNotifications(
        ...
        try {
            ...
            return BulkNotificationResponse.failure("Erro ao processar arquivos: " + e.getMessage(), List.of());
        } catch (Exception e) {
    ...
    private BulkNotificationResponse sendToAccounting(
        ...
        try {
            ...
            boolean success = switch (channel) {
                case EMAIL -> notificationCoordinator.sendEmailOnly(
                ...
                case WHATSAPP, BOTH -> {
                    ...
                    NotificationResponse response = notificationCoordinator.sendNotification(
                            accountingEmail,
                            accountingWhatsapp,
                            channel,
                            subject,
            ...
            if (success) {
                ...
                return BulkNotificationResponse.success(
                        1,
                        String.format("%d arquivo(s) enviado(s) para contabilidade via %s", 
                                fileContents.size(), channelText)
                );
            } else {
                return BulkNotificationResponse.failure(
                        "Falha ao enviar para contabilidade",
                        List.of("Contabilidade")
                );
            }
            ...
            return BulkNotificationResponse.failure(
                    "Erro ao enviar para contabilidade: " + e.getMessage(),
                    List.of("Contabilidade")
            );
        }
    }
    ...
    private BulkNotificationResponse sendToClients(
        ...
        for (Long clientId : clientIds) {
            try {
                ...
                if (channel == NotificationChannel.EMAIL && (client.getEmail() == null || client.getEmail().isEmpty())) {
                    log.warn("Cliente {} não possui e-mail cadastrado", client.getClientName());
                    failedRecipients.add(client.getClientName() + " (sem e-mail)");
                    continue;
                }

                if (channel == NotificationChannel.WHATSAPP && (client.getPhoneNumber() == null || client.getPhoneNumber().isEmpty())) {
                ...
                if (channel == NotificationChannel.BOTH) {
                    if ((client.getEmail() == null || client.getEmail().isEmpty()) && 
                        ...
                        log.warn("Cliente {} não possui e-mail nem telefone cadastrado", client.getClientName());
        ...
        if (successCount == 0) {
            ...
            return BulkNotificationResponse.success(
                    successCount,
                    String.format("%d arquivo(s) enviado(s) com sucesso para %d destinatário(s) via %s",
                            fileContents.size(), successCount, channelText)
            );
        } else {
            return BulkNotificationResponse.partial(successCount, failedRecipients.size(), failedRecipients);
        }
    }
