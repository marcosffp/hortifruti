...
public class FileGenerationService {
  ...
  public byte[] createZipWithStatements(int month, int year) throws IOException {
    ...
    String tempZipPath = System.getProperty("java.io.tmpdir") + "/statements_" + month + "_" + year + ".zip";
    ...
    try (ZipFile zipFile = new ZipFile(tempZipPath)) {
      ...
      List<Statement> bbStatements = statements.stream()
      ...
      if (!bbStatements.isEmpty()) {
        ...
        String bbExcelPath = System.getProperty("java.io.tmpdir") + "/extrato_bb_" + month + "_" + year + ".xlsx";
      ...
      List<Statement> sicoobStatements = statements.stream()
          .filter(s -> s.getBank() == Bank.SICOOB)
          .collect(Collectors.toList());
      if (!sicoobStatements.isEmpty()) {
        byte[] sicoobExcel = generateBankExcel(sicoobStatements, "Sicoob");
        String sicoobExcelPath = System.getProperty("java.io.tmpdir") + "/extrato_sicoob_" + month + "_" + year + ".xlsx";
      ...
      if (invoicesZip.length > 0) {
        String invoicesZipPath = System.getProperty("java.io.tmpdir") + "/notas_fiscais_" + month + "_" + year + ".zip";
    ...
    try (Workbook workbook = new XSSFWorkbook()) {
      ...
      for (Statement statement : statements) {
        for (Transaction transaction : statement.getTransactions()) {
          ...
          row.createCell(2).setCellValue(transaction.getDocument() != null ? transaction.getDocument() : "");
  ...
  public byte[] processGenericFiles(List<byte[]> files, List<String> fileNames, 
                                   String debitValue, String creditValue, String cashValue) throws IOException {
  ...
  public byte[] generateMonthlyInvoicesZip(int month, int year) throws IOException {
    try {
      ...
      String tempZipPath = System.getProperty("java.io.tmpdir") + "/invoices_" + month + "_" + year + ".zip";
      ...
      try (ZipFile zipFile = new ZipFile(tempZipPath)) {
        ...
        for (int i = 1; i <= 5; i++) {
          ...
          String invoicePath = System.getProperty("java.io.tmpdir") + "/nota_fiscal_" + i + "_" + month + "_" + year + ".xlsx";
  ...
  private byte[] generateSampleInvoice(int invoiceNumber, int month, int year) throws IOException {
    try (Workbook workbook = new XSSFWorkbook()) {
      ...
      numberRow.createCell(0).setCellValue("Número:");
      numberRow.createCell(1).setCellValue("NF-" + year + month + String.format("%03d", invoiceNumber));
