...
public interface ChatSessionRepository extends JpaRepository<ChatSession, Long> {
    ...
    /**
     * Busca a sessão ativa mais recente de um número de telefone
     * (simplificado: qualquer sessão existente é ativa, sessões concluídas são deletadas)
     */
    @Query("SELECT cs FROM ChatSession cs WHERE cs.phoneNumber = :phoneNumber " +
     ...
     */
    @Query("SELECT cs FROM ChatSession cs WHERE cs.status = 'AWAITING_HUMAN' " +
     ...
     */
    @Query("SELECT cs FROM ChatSession cs WHERE cs.pausedUntil IS NOT NULL " +
           "AND cs.pausedUntil < :now")
    List<ChatSession> findSessionsToUnpause(@Param("now") LocalDateTime now);

    /**
     * Conta quantas sessões ativas um cliente possui
     * (simplificado: todas as sessões existentes são ativas)
     */
     ...
     * (simplificado: ordena por createdAt ao invés de updatedAt)
     */
