...
public InvoiceResponse issueInvoice(Long combinedScoreId, String dadosAdicionais) {
  try {
    System.out.println("[IssueInvoice] Iniciando emissão de NF para combinedScoreId: " + combinedScoreId);

    CombinedScore combinedScore = fetchCombinedScore(combinedScoreId);
    System.out.println("[IssueInvoice] CombinedScore encontrado: id=" + combinedScore.getId() + ", clientId=" + combinedScore.getClientId());
...
private IssueInvoiceRequest buildInvoiceRequest(
  ...
  System.out.println("[buildInvoiceRequest] Construindo request para clientId=" + combinedScore.getClientId());

  Client client = fetchClient(combinedScore.getClientId());
  String firstName = client.getClientName().split("\\s+")[0].toUpperCase().trim();
  System.out.println("[buildInvoiceRequest] firstName=" + firstName + ", dadosAdicionais=" + dadosAdicionais);
