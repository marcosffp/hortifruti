...
public interface StatementRepository extends JpaRepository<Statement, Long> {
   ...
   */
  List<Statement> findByBankAndCreatedAtBetween(Bank bank, LocalDateTime startDate, LocalDateTime endDate);
  ...
  /**
   * Busca statements que tenham transações no período especificado
   * Esta query é mais complexa e busca statements que tenham pelo menos uma transação no período
   */
  @Query("SELECT DISTINCT s FROM Statement s JOIN s.transactions t " +
  ...
  /**
   * Busca o statement que melhor cobre um período específico
   * Retorna o statement com maior número de transações no período solicitado
   */
  @Query("SELECT s FROM Statement s JOIN s.transactions t " +
         "WHERE s.bank = :bank AND t.transactionDate BETWEEN :startDate AND :endDate " +
         "GROUP BY s.id " +
