...
import com.hortifruti.sl.hortifruti.service.notification.email.EmailService;
...
public class CombinedScoreOverdueService {
  ...
  /**
   * Verifica CombinedScores vencidos e envia notificações. Este método será chamado manualmente via
   * endpoint.
   */
  ...
  /**
   * Lógica principal para verificação de CombinedScores vencidos Separada para permitir uso tanto
   * automático quanto manual
   */
  public List<CombinedScore> checkOverdueCombinedScores() {
    ...
    try {
      ...
      // Um boleto com dueDate = "2025-01-11" será considerado vencido a partir de "2025-01-12
      // 00:00:00"
      ...
      if (!overdueScores.isEmpty()) {
        ...
        log.info(
            "Enviando notificações para {} clientes com CombinedScores vencidos",
            scoresByClient.size());
  ...
  /**
   * Busca apenas os CombinedScores vencidos sem enviar notificações Útil para consultas e testes
   */
  public List<CombinedScore> getOverdueScoresOnly() {
    ...
    try {
      ...
      if (overdueScores.isEmpty()) {
        ...
        overdueScores.forEach(
            score ->
                log.info(
                    "Boleto vencido encontrado - ID: {}, ClientID: {}, DueDate: {}, ConfirmedAt: {}",
                    score.getId(),
                    score.getClientId(),
                    score.getDueDate(),
                    score.getConfirmedAt()));
      }
  ...
  private void sendHtmlEmailDirectly(String email, String subject, String htmlBody) {
    try {
      ...
      log.info(
          "Conteúdo do email seria enviado com subject: {} e {} caracteres de body",
          subject,
          htmlBody.length());
    }
  }
  ...
  private String buildManagementOverdueHtmlBody(List<CombinedScore> overdueScores) {
    try {
      ...
      scoresByClient.forEach(
          (clientId, clientScores) -> {
            var client = clientRepository.findById(clientId);
            String clientName =
                client.map(c -> c.getClientName()).orElse("Cliente ID: " + clientId);
            ...
            if (clientScores.size() == 1) {
              ...
              clientRows
                  .append("<td><span class=\"boleto-count\">")
                  .append(clientScores.size())
                  .append(" boletos</span></td>");
            }

            clientRows
                .append("<td class=\"amount\">R$ ")
                .append(String.format("%.2f", clientTotal))
                .append("</td>");
            clientRows.append("</tr>");
          });
  ...
  private String buildManagementOverdueFallbackBody(List<CombinedScore> overdueScores) {
    ...
    body.append("<p><strong>Data:</strong> ")
        .append(LocalDateTime.now().toLocalDate())
        .append("</p>");
    body.append("<br>");
    ...
    body.append("<li>Valor total: R$ ")
        .append(String.format("%.2f", totalOverdueAmount))
        .append("</li>");
    body.append("</ul>");
    body.append("<br>");
