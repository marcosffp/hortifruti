...
public class Base64FileDecoder {
    ...
    public File decodeGoogleDriveCredentials() throws IOException {
        if (googleDriveCredentials == null || googleDriveCredentials.isEmpty()) {
            System.err.println("[ERROR] A propriedade 'google.drive.credentials' está vazia ou não foi configurada.");
            throw new IllegalArgumentException("A propriedade 'google.drive.credentials' está vazia ou não foi configurada.");
        }
        System.out.println("[DEBUG] Decodificando credenciais do Google Drive para o diretório: " + googleTempDirectory);
        ...
        System.out.println("[DEBUG] Arquivo de credenciais decodificado: " + decodedFile.getAbsolutePath());
        return decodedFile;
    }
    ...
    public File decodePfx() throws IOException {
        if (pfx == null || pfx.isEmpty()) {
            throw new IllegalArgumentException("A propriedade 'document.pfx' está vazia ou não foi configurada.");
        }
        ...
        if (decodedFile.exists()) {
            System.out.println("[DEBUG] Arquivo PFX decodificado com sucesso: " + decodedFile.getAbsolutePath());
        } else {
