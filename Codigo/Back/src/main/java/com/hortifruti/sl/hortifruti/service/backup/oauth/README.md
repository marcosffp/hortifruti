...
Implementa o fluxo OAuth2 "authorization code" do Google usado para a primeira autorização (ou reautorização) de acesso ao Drive/Gmail: recebe o código de autorização do callback HTTP, troca por tokens e os persiste no MySQL via `DatabaseDataStoreFactory` (ver `backup/auth`). Complementar a `backup/auth`, que cuida da validação/renovação de credenciais já obtidas.
