...
|---|---|---|
| `GoogleAuthService.java` | `@Service` | Monta o cliente `Drive` autenticado a partir de `CredentialConfig`/`CredentialManager` (`service.googleauth`); propaga como `BackupException("AUTHORIZATION_REQUIRED:...")` quando o usuário precisa reautorizar. |
