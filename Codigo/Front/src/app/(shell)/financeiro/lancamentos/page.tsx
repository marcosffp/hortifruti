...
import { Search, Plus, Download, ArrowUp, ArrowDown, Calendar, Edit, Trash2, ArrowLeft, ArrowRight } from 'lucide-react';
...
export default function FinancialLaunchesPage() {
    const { isLoading, error, getTotalRevenue, getTotalExpenses, getTotalBalance, getAllTransactions, deleteTransaction, exportTransactionsAsExcel, getAllCategories } = useTransaction();
    ...
    const fetchData = async () => {
        try {
            ...
            const allTransactions: PageResult<TransactionResponse> | undefined = await getAllTransactions(search, type, category, page);
    ...
    return (
        <main className="flex-1 p-6 bg-gray-50 overflow-auto flex flex-col">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-600">Gerencie todos os lançamentos financeiros do sistema</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 flex items-center justify-between">
                    <div>
                        ...
                        {isLoading ? (
                            <h2 className="text-2xl font-bold text-green-600">Carregando...</h2>
                        ) : error ? (
                            <h2 className="text-2xl font-bold text-red-600">Erro</h2>
                        ) : (
                            <h2 className="text-2xl font-bold text-green-600">R$ {totalRevenue?.toFixed(2).replace(".", ",")}</h2>
                        )}
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 flex items-center justify-between">
                    <div>
                        ...
                        {isLoading ? (
                            ...
                            <h2 className="text-2xl font-bold text-red-600">R$ {totalExpenses?.toFixed(2).replace(".", ",")}</h2>
                        )}
                ...
                <div className="bg-white rounded-lg shadow-sm p-6 flex items-center justify-between">
                    <div>
                        ...
                        {isLoading ? (
                            ...
                            <h2 className="text-2xl font-bold text-gray-800">R$ {totalBalance?.toFixed(2).replace(".", ",")}</h2>
                        )}
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 flex-grow flex flex-col">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-xl font-semibold text-gray-800">Lista de Lançamentos</h2>
                        <p className="text-sm text-gray-500">{transactions?.length || 0} lançamento(s) encontrado(s)</p>
                    </div>
                    <div className="flex space-x-4">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                        >
                            {isLoading ? "Exportando..." : <><Download size={18} className="mr-2" />Exportar</>}
                        </button>
                    </div>
                </div>
                            ...
                            <tbody>
                                {transactions.map((transaction) => (
                                    ...
                                    <tr key={transaction.id} className="border-b border-gray-100">
                                        ...
                                        <td className="py-3 px-4"><span className="border border-[var(--neutral-300)] text-xs font-medium px-2.5 py-0.5 rounded-full ">{transaction.category}</span></td>
                                        <td className="py-3 px-4">
                                            <span
                                            ...
                                            >
                                                {transaction.transactionType === "CREDITO" ? "Entrada" : "Saída"}
                                            </span>
                                        </td>
                                        <td
                                            className={`py-3 px-4 ${transaction.transactionType === "CREDITO" ? "text-green-600" : "text-red-600"}`}
                                        >
                                            {`${transaction.transactionType === "CREDITO" ? "+" : "-"}R$ ${(transaction.amount || 0).toFixed(2).replace(".", ",")}`}
                                        </td>
                ...
                <div className="mt-4 flex justify-start gap-5">
                    ...
                    >
                        ...
                        >
                            <ArrowLeft size={20} className={page === 0 ? "text-gray-400" : "text-gray-700"} />
                        </span>
                        Anterior
                    </button>
                    <button
                    ...
                    >
                        Próxima
                        <span
                            className={`flex items-center justify-center w-8 h-8 rounded-full ${page === totalPages - 1 ? "bg-gray-300" : "bg-gray-100 hover:bg-gray-200"
                                }`}
                        >
                            <ArrowRight size={20} className={page === totalPages - 1 ? "text-gray-400" : "text-gray-700"} />
                        </span>
                    </button>
                </div>
            </div>
