...
O frontend do **Hortifruti SL** é uma SPA em **Next.js (App Router) + TypeScript** que dá acesso visual a toda a operação gerenciada pelo backend: dashboard financeiro com gráficos de fluxo de caixa, cadastro e acompanhamento de clientes e compras, conciliação de extratos bancários, cálculo de frete com mapas interativos (Leaflet/OSRM e Google Places), geração e consulta de boletos e notas fiscais eletrônicas, central de notificações (e-mail/WhatsApp), backup do banco de dados e administração de usuários — tudo protegido por autenticação JWT e controle de acesso por papéis (**Gestor**, **Funcionário**, **Contador**).

---

## 🏛️ Arquitetura

A aplicação segue o modelo de rotas do **App Router** do Next.js, separando páginas públicas (login, landing, acesso negado) de um *layout* autenticado (*route group* `(shell)`) que aplica guarda de sessão e papéis a todas as telas internas.
...
│   (shell)/...                          (autenticadas)          │
│       └─ AuthGuard → RoleGuard → Sidebar + Header + página     │
└───────────────────────────┬────────────────────────────────────┘
                            │ hooks (estado, loading, erro)
┌───────────────────────────▼────────────────────────────────────┐
│                          services/                             │
│   fetch/axios + Bearer token (httpUtils) → API REST do backend │
└─────────────────────────────────────────────────────────────────┘
```
...
|---|---|
| Guarda de rota (`AuthGuard`) | Redireciona para `/login` quando não há sessão válida |
| Guarda de papel (`RoleGuard`) | Redireciona para `/acesso-negado` quando o usuário não tem o papel exigido pela tela |
...
| `services` + `httpUtils` | Centralizam a comunicação HTTP, anexando o token JWT (`Authorization: Bearer`) a cada chamada |
| Token JWT decodificado no cliente | `authService` decodifica o JWT e guarda usuário/papéis em `localStorage` |
...
│   │   ├── modules/                  # CashFlow, Map, ClientCard, UploadExtract, tables/…
│   │   └── landing/                  # Componentes da página de apresentação
│   ├── hooks/                        # useAuth, useClient, useDashboard, useTransaction…
...
|---|---|---|
| **Dashboard** | `CashFlow` (gráficos Chart.js: linha, barra, pizza) | Fluxo de caixa mensal, receitas por tipo, categorias de gasto, top produtos, formatação em R$ |
...
| **Boletos** | `BilletsTable`, `ShowBilletModal`, `ShowBilletDataModal` | Geração, 2ª via, cancelamento e consulta de boletos Sicoob |
| **Notas fiscais** | `NotesTable`, `ShowInvoiceModal`, `ShowInvoiceDataModal` | Emissão, DANFE, XML e relatórios fiscais |
...
A comunicação com a API é centralizada em `src/services`, que usa os helpers de `src/utils/httpUtils.ts` (`getAuthHeaders`, `fetchWithAuth`, `getAuthHeadersForFormData`) para anexar o token JWT salvo em `localStorage` a cada requisição — incluindo uploads `multipart/form-data`. Cada domínio tem um hook (`src/hooks`) que encapsula estado de carregamento/erro sobre o respectivo `service`:

| Hook | Service | Responsabilidade |
|---|---|---|
| `useAuth` | `authService` | Login, logout, decodificação do JWT, verificação de papéis (`hasRole`) |
...
| `useBillet` | `billetService` | Geração, consulta, 2ª via e cancelamento de boletos |
| `useInvoice` | `invoiceService` | Emissão, consulta, DANFE, XML e cancelamento de NF-e |
...
```dotenv
# URL da API do backend (padrão local: http://localhost:8080)
...
- ⚠️ **`NEXT_PUBLIC_API_URL`** deve apontar para o backend correspondente (local ou produção)
...
- 🔑 O token JWT é decodificado e mantido em `localStorage` pelo `authService` — papéis e expiração são checados a cada navegação
...
| Build | `npm run build` |
| Variáveis de ambiente | Configuradas no painel da Vercel (mesmas do `.env` local) |
